...
- `tests/snapshots/openapi.json` is a golden snapshot asserted byte-for-byte by `tests/test_openapi_snapshot.py`; it guards consumer projects' OpenAPI against route/schema/typing drift. Keep `just test` green. When the spec changes on purpose, regenerate it with `just gen-openapi` and review the diff — never edit it to make the test pass.
- Integration tests need a reachable PostgreSQL (user able to CREATE DATABASE/EXTENSION): the schema is built via Alembic migrations into a `<db>-test-tpl` template DB, cloned per pytest-xdist worker, with `TRUNCATE` per test. They skip when Postgres is unavailable; the snapshot test needs no DB. Test models live in `tests/models/` (one model per file).
