...
async def notify_user(user_id: int, message: str, tasks: QueuedTasks = Inject(QueuedTasks)):
...
@router.post("/orders")
async def create_order(order_data: dict, tasks: QueuedTasks = Inject(QueuedTasks)):
...
async def process_user_gallery(user_id: int, tasks: QueuedTasks = Inject(QueuedTasks)):
