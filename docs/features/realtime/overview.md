...
- **Scoped to what is being read**: a worker only hears about the scopes it holds a socket
  for, and a socket only hears about the records it asked about.
...
to every client reading its scope:
...
- **Metadata a client caches**: custom fields declared for a scope, so a column removed
