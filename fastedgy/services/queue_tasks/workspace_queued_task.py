...
from fastedgy.queued_task.services.queue_hooks import on_pre_create, on_pre_run, on_post_run
...
async def restore_workspace_context(task) -> None:
    ...
    if workspace_id and user_id:
        context.set_workspace_user(await WorkspaceUser.query.filter((WorkspaceUser.columns.user == user_id) & (WorkspaceUser.columns.workspace == workspace_id)).get_or_none())
