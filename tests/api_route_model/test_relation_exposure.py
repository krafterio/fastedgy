...
def _generated_models(model_cls):
    return (
        generate_output_model(model_cls),
        generate_input_create_model(model_cls),
        generate_input_patch_model(model_cls),
    )
...
def test_auto_generated_reverse_relation_is_hidden(setup_openapi_app: FastEdgy) -> None:
    ...
    for generated in _generated_models(QueuedTask):
...
def test_disabled_reverse_relation_is_hidden(setup_openapi_app: FastEdgy) -> None:
    ...
    for generated in _generated_models(Workspace):
...
def test_explicit_and_m2m_relations_are_exposed(setup_openapi_app: FastEdgy) -> None:
    for generated in _generated_models(Product):
        assert "tags" in generated.model_fields

    for generated in _generated_models(Category):
